...
| PA-22 | La clave de Plus es una categoría que agrupa materias (no 1:1 con comisión), vive en `Materia.clave_plus`, fija/precargada por producto vía migración (no editable por el ADMIN del tenant). Mapeo vigente: PROG1/PROG2→PROG, BD1→BD, MAT1→MAT, ING1→ING | [05_reglas_de_negocio.md RN-33](05_reglas_de_negocio.md#rn-33), `backend/alembic/versions/017_materia_clave_plus.py` |
| PA-23 | Se acumula N × Plus(clave, rol) sin tope, igual para todos los roles (ya era la regla en RN-33/RN-34; lo que estaba roto era que el código calculaba el Plus sobre las letras de comisión en vez de la clave de la materia — corregido en `LiquidacionService.calcular`) | [05_reglas_de_negocio.md RN-33](05_reglas_de_negocio.md#rn-33), `backend/app/services/liquidaciones.py` |
